...
class PaymentHandler {
	...
	public function __construct( $configPath, $paymentHandlerConfig = null ) {
		if ( $paymentHandlerConfig != null ) {
			...
			$this->paymentHandlerConfig = PaymentHandlerConfig::getInstance()->withInstance( $merchantId, $apiKey, $paymentPageClientId, $baseUrl, $enableLogging, $loggingPath, $responseKey );
		}
	}
	...
	public function orderStatus( $orderId ) {
		return PaymentEntity::makeServiceCall( "/orders/{$orderId}", null, RequestMethod::GET );
	}
	...
	public function orderSession( $params ) {
		...
		return PaymentEntity::makeServiceCall( "/session", $params, RequestMethod::POST, ContentType::JSON );
	...
	public function refund( $params ) {
		$this->paramsCheck( $params );
		return PaymentEntity::makeServiceCall( "/refunds", $params, RequestMethod::POST, ContentType::X_WWW_FORM_URLENCODED );
	}
...
class PaymentEntity {
	 ...
	 * @throws APIException
	 */
	public static function makeServiceCall( $path, $params, $method, $contentType = null ) {
		$logger = new SimpleLogger();
...
class PaymentHandlerConfig {
	 ...
	 */
	public function withInstance( $merchantId, $apiKey, $paymentPageClientId, $baseUrl, $enableLogging, $loggingPath, $responseKey ) {
		$this->apiKey = $apiKey;
...
class SimpleLogger {
	...
	public function log( $level, $message ) {
		...
		if ( PaymentHandlerConfig::getInstance()->getEnableLogging() ) {
			...
			$formattedMessage = sprintf( "[%s] in.PaymentHandler.%s [%s]: context:%s %s", $formattedTimestamp, $callerFunction, strtoupper( $level ), $this->context, $message );
