...
(function () {
  ...
  var defaultAtomicTagsRegExp = new RegExp(
    "^<(iframe|object|math|svg|script|video|head|style|a)"
  );
  ...
  function isWrappable(token) {
    ...
    return (
      is_img || isntTag(token) || isStartOfAtomicTag(token) || isVoidTag(token)
    );
  }
  ...
  var OPS = {
    ...
    replace: function () {
      return (
        OPS["delete"].apply(null, arguments) +
  ...
  function diff(before, after, className, dataPrefix, atomicTags) {
    ...
    atomicTags
      ? (atomicTagsRegExp = new RegExp(
          "^<(" + atomicTags.replace(/\s*/g, "").replace(/,/g, "|") + ")"
        ))
